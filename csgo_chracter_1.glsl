...
//: param custom { "default": false, "label": "Light Warp(Bugy!!!)", "group": "Warp Parameters" }
...
uniform float g_fPhongBoost;

//: param custom { "default": 1.0, "label": "$PhongAlbedoBoost", "min": 0.0, "max": 255.0, "group": "Phong Parameters" }
uniform float g_fPhongAlbedoBoost;

//: param custom { "default": 5.0, "label": "$PhongExponent", "min": 1.0, "max": 150.0, "group": "Phong Parameters" }
...
//: param custom { "default": false, "label": "Use Ambient Reflection", "group": "Ambient Reflection Parameters" }
...
uniform float g_fAmbientBounceBoost;
...
//: param custom { "default": [0.0, 42.0, 0.0], "label": "$AmbientReflectionBounceCenter", "group": "Ambient Reflection Parameters" }
...
//#define g_vViewFakeRimDir vec3( 0.7f, 1.0f, 0.0f )
...
uniform float g_fShadowRimBoost;
...
//: param custom { "default": 0.0, "label": "$EnvmapContrast", "min": -1.0, "max": 1.0, "group": "Envmap Parameters" }
...
uniform float g_fSelfIllumBoost;
...
void PropagateLights(vec3 worldPos) //there are all light uniform parameter ,it is easy to load all set
{
	...
	if (bIBL)
	{
		...
		if (!bIBL_Color)
			g_cLightInfo[0].color.rgb   = LinearToGamma(SunlightColor_0 * r_lightBoost);
		else
			g_cLightInfo[0].color.rgb	= LinearToGamma(envIrradiance(light_main.xyz - worldPos) * r_lightBoost);
	...
	g_cLightInfo[1].color.rgb   = LinearToGamma(lightColor_1 * r_lightBoost);
	g_cLightInfo[1].pos.xyz     = (lightPosition_1);
	g_cLightInfo[2].color.rgb   = LinearToGamma(lightColor_2 * r_lightBoost);
	g_cLightInfo[2].pos.xyz     = (lightPosition_2);

	vec3 temp_color = LinearToGamma(lightColor_3 * r_lightBoost);
						  ...
						  out vec3 specularLighting, out vec3 rimLighting, out float rimHalo )
{
	...
	if ( bDoSpecularWarp )
	{
		specularLighting = texture( specularWarpSampler, vec2( min(flNDotH,0.99f), fWarpIndex ) ).rgb; //0.99f fix dark Dot
	}
	else
	{
...
//combine shader parameters for csgo specific
    ...
    // vec4 g_vDiffuseModulation; //what is this?
	...
	cAmbient_fRimBoost.a = smoothstep( g_vShadowRimMax, g_vShadowRimMin, fAmbientLightLum );//it is too small?
	...
	//vBounceCenterDir_vBounceCenterz.xyz 	= normalize( g_vBounceCenter - ( vec4(inputs.position,1) * uniform_camera_vp_matrix_inverse ).xyz); //BounceCenter - app_position
	vBounceCenterDir_vBounceCenterz.xyz 	= normalize( g_vBounceCenter.xyz - inputs.position );
	//vBounceCenterDir_vBounceCenterz.xyz 	= normalize( ( vec4(g_vBounceCenter,1) * uniform_camera_view_matrix_it ).xyz - inputs.position ); //substance bug,cant load uniform_camera_vp_matrix_inverse
	//vBounceCenterDir_vBounceCenterz.xyz 	= g_vBounceCenter - g_vModelOrigin; // Substance Painter Obj Origin should be same as World Origin
	vWorldTangentS_vBounceCenterx.w 		= g_vBounceCenter.z; //swap Z and X for match source engine
	vWorldTangentT_vBounceCentery.w 		= g_vBounceCenter.y;
	vBounceCenterDir_vBounceCenterz.w 		= g_vBounceCenter.x;
...
*/
    ...
    vec3 vWorldNormal 	= computeWSNormal(inputs.sparse_coord, inputs.tangent, inputs.bitangent, inputs.normal);
	...
	//fCSMShadow = pow(fCSMShadow,100); //boost shadow
...
*/
	...
	if ( FRESNELRANGESTEXTURE )
    {
		...
		vec3 vFresnelParams = texture ( FresnelRangesSampler, vec2( min(fFresnel,0.99f), fWarpIndex ) ).rgb;
    ...
    vMasks2Params.b = sampleWithDefault(LightScaleSampler               , inputs.sparse_coord, g_fEnvmapLightScale);
	...
	if ( MASKS2 )
    {
		...
		fAnisotropyAngle = vMasks2Params.g * M_PI;
...
*/
	...
	vec3 vReflectionEyeVec = vEyeDir;
	float fRimHalo = 0;

	if ( FAKERIM ) // Fake Rim Light is a Staic Top Front Light,but just getting the Rimlight term :D
	{
		...
		//vec3 vFakeRimDir =  normalize( (  vec4( g_vViewFakeRimDir, 1 ) * uniform_camera_view_matrix_it ).xyz );//need mvp?
		vec3 vFakeRimDir =  normalize( g_vViewFakeRimDir );//it is a light dir
		vec3 cFakeRimColor = cAmbient_fRimBoost.rgb * cAmbient_fRimBoost.a * g_vFakeRimTint_ShadowScale.xyz;//most close
	...
	if ( ENVMAP )
	{
		vec3 cEnvmap = envSampleLOD (vReflection, 2);

		cEnvmap = saturateColor( cEnvmap, g_fEnvmapSaturation );

		vec3 cEnvmapLight = saturate( ( ( linearLightColor + cAmbient_fRimBoost.xyz ) - g_vEnvmapLightScaleMin ) * g_vEnvmapLightScaleMax ) * fCSMShadow;//shadow fix
	...
	if ( SHADOWSATURATION )
	{
		...
		cBase = mix( cBase, saturateColor( cBase, g_fShadowSaturation ), fShadeLevelsDark * fShadeLevelsLight * fShadowSaturationMask );
